...
def compute_crystal_growth(infile: Path, outfile: Path) -> None:
    ...
    with gsd.hoomd.open(str(infile)) as traj:
        for snap in traj:
            ...
            clustering_matrix = np.append(snap.particles.position,
                                          crystalline*order_dimension,
                                          axis=1)

            _, labels = cluster.dbscan(clustering_matrix, min_samples=4, eps=3,
                                       metric=partial(periodic_distance,
                                                      np.append(snap.configuration.box[:3],
                                                                order_dimension*2)))
...
@click.command()
@click.option('-i', '--input-path', default=None,
